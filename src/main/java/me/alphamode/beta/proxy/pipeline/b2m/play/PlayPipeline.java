...
//		TODO: S2CInitializeBorderPacket
	...
	public void handleS2CAddPainting(final ClientConnection connection, final AddPaintingPacket packet) {
		connection.send(new S2CBundleDelimiterPacket());
		connection.send(new S2CAddEntityPacket(
				packet.entityId(),
				UUID.randomUUID(),
				ModernEntityTypes.PAINTING,
	...
	public void handleS2CAnimate(final ClientConnection connection, final AnimatePacket packet) {
		switch (packet.action()) {
			...
			case AnimatePacket.Action.DAMAGE_ANIMATION ->
					connection.send(new S2CHurtAnimationPacket(packet.entityId(), 0.0F)); // TODO/FIX
