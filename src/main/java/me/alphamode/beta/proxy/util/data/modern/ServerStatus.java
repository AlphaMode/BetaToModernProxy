...
public record ServerStatus(TextComponent description,
    ...
    public record Favicon(byte[] iconBytes) {
        public static final Codec<Favicon> CODEC = Codec.STRING.flatMap(favicon -> {
            return Result.success("data:image/png;base64," + new String(Base64.getEncoder().encode(favicon.iconBytes), StandardCharsets.UTF_8));
        }, data -> {
            if (!data.startsWith("data:image/png;base64,")) {
                return Result.error("Unknown format");
            } else {
                try {
                    String base64 = data.substring("data:image/png;base64,".length()).replaceAll("\n", "");
